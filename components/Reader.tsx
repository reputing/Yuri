...
export function ReaderModal({ gallery, initialPage = 1, onClose }: { gallery: NHGallery; initialPage?: number; onClose: () => void }) {
  ...
  const go = useCallback((delta: number) => {
    setCurrent(prev => {
      ...
      if (next < 1 || next > total) return prev
      setImgLoading(true)
      return next
    })
  }, [total])
  ...
  return (
    <div className="fixed inset-0 z-[2000] bg-black flex flex-col">
      <div className="flex items-center gap-3 px-4 h-[52px] bg-black/90 border-b border-border/50 flex-shrink-0">
        <button onClick={onClose} className="w-9 h-9 rounded-md bg-bg4 border border-border flex items-center justify-center text-muted hover:bg-red-500 hover:text-white hover:border-red-500 transition-all flex-shrink-0">
