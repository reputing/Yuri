...
export function coverUrl(g: NHGallery): string {
  if (g.images.cover.url) return g.images.cover.url
  return `https://t.nhentai.net/galleries/${g.media_id}/cover.${EXT[g.images.cover.t] ?? 'jpg'}`
}

export function thumbnailUrl(g: NHGallery): string {
  if (g.images.thumbnail.url) return g.images.thumbnail.url
  return `https://t.nhentai.net/galleries/${g.media_id}/thumbnail.${EXT[g.images.thumbnail.t] ?? 'jpg'}`
}

export function pageUrl(mediaId: string, pageNum: number, ext: ImageType): string {
  return `https://i.nhentai.net/galleries/${mediaId}/${pageNum}.${EXT[ext] ?? 'jpg'}`
}

export function thumbPageUrl(mediaId: string, pageNum: number, ext: ImageType): string {
  return `https://t.nhentai.net/galleries/${mediaId}/${pageNum}t.${EXT[ext] ?? 'jpg'}`
}
