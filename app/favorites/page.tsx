  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-8">
        ...
        <span className="ml-3 text-lg font-normal text-faint">({favorites.length})</span>
      </h1>
            ...
            <a key={fav.id} href={`/gallery/${fav.gallery_id}`} className="block">
              <div className="bg-card border border-border rounded-xl overflow-hidden card-hover">
                <div className="aspect-[7/10] bg-bg4 relative">
                  {fav.gallery_cover ? (
                    <img
                      src={fav.gallery_cover}
                      alt={fav.gallery_title ?? ''}
                  ...
                  <div className="absolute top-1.5 right-1.5 text-xs font-semibold px-1.5 py-0.5 rounded bg-black/70 text-muted border border-border">
                    {fav.gallery_num_pages}p
                  </div>
                </div>
                <div className="p-2.5">
                  <p className="text-xs font-medium leading-snug line-clamp-2 text-[#e8e8f0]">
                    {fav.gallery_title}
                  </p>
                </div>
              </div>
